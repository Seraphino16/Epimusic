...
const ProductAdminAddModel = () => {
    ...
    const [weight, setWeight] = useState(""); // État pour le poids
    ...
    const getCategoryId = async (category) => {
        try {
            const response = await fetch(`http://localhost:8000/api/admin/get/catergoryId?name=${category}`);
            if (!response.ok) {
            ...
            setAlert({ type: 'error', message: 'Erreur lors de la récupération de la catégorie' });
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const uploadedPhotos = await Promise.all(photoFiles.map(file => uploadImage(file)));
            ...
            const response = await fetch(`http://localhost:8000/api/admin/addModel`, {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                ...
                setAlert({ type: 'success', message: 'Modèle créé avec succès' });
                navigate(`/admin`);
            }
        } catch (error) {
            console.log('Erreur:', error);
            setAlert({ type: 'error', message: 'Erreur lors de la création du modèle' });
        } finally {
        ...
        <div>
            ...
            <form onSubmit={handleSubmit}>
                {shouldDisplayColor(categoryId) && (
                    <div className="md:flex items-center mt-8">
                        <div className="w-full flex flex-col">
                            <label className="font-semibold leading-none text-black" htmlFor="color">Couleur</label>
                            <select
                                id="color"
                                value={color}
                ...
                {shouldDisplaySize(categoryId) && (
                    <div className="md:flex items-center mt-8">
                        <div className="w-full flex flex-col">
                            <label className="font-semibold leading-none text-black" htmlFor="size">Taille</label>
                            <select
                                id="size"
                                value={size}
                ...
                <div className="md:flex items-center mt-8">
                    <div className="w-full flex flex-col">
                        <label className="font-semibold leading-none text-black" htmlFor="price">Prix</label>
                        <input
                            type="text"
                            id="price"
                ...
                <div className="md:flex items-center mt-8">
                    <div className="w-full flex flex-col">
                        <label className="font-semibold leading-none text-black" htmlFor="stock">Stock</label>
                        <input
                            type="number"
                            id="stock"
                ...
                <div className="md:flex items-center mt-8">
                    <div className="w-full flex flex-col">
                        <label className="font-semibold leading-none text-black" htmlFor="weight">Poids</label>
                        <input
                            type="text"
                            id="weight"
                ...
                <div className="flex items-center justify-center w-full mt-8">
                    <button
                        ...
                        className={`font-semibold leading-none text-white py-4 px-10 rounded focus:ring-2 focus:ring-offset-2 focus:ring-blue-700 focus:outline-none ${isSubmitting ? 'bg-gray-400' : 'bg-blue-700 hover:bg-blue-600'}`}
                    >
